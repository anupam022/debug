...
// Corrected declaration of struct ftpc_options
struct ftpc_options o;
...
enum ErrorCodes
{
    ...
    ERR_LOST_WORKING_DIRECTORY,
    ERR_DOUBLE_SIGINT,
    ERR_NO_USABLE_TMPDIR
};
...
void print_progress(char *op, char *fn, size_t fn_len, off_t cur_pos, off_t total_size)
{
    ...
    size_t line_len;
    double progress = total_size > 0 ? (cur_pos * 100.0 / total_size) : 100.0;
    ...
    snprintf(line, WIDE_LINE_LEN, "%s %lld%s/%lld%s (%4.*f%%) ",
             op, cur_pos, units[u_idx], total_size, units[u_idx],
             progress < 1.0 ? 2 : 1, progress);

    line[79] = '\0'; // Ensuring null termination
    line_len = strlen(line);

    if (line_len + fn_len < WIDE_LINE_LEN)
    {
        strcpy(line + line_len, fn);
    }
    else
    {
        sprintf(line + line_len, "[...]%s", (fn + fn_len - (WIDE_LINE_LEN - line_len)));
    }
...
int local_file(char *name)
{
    ...
    return (S_ISREG(buf.st_mode) ? 1 : S_ISDIR(buf.st_mode) ? 2
                                                            : 3);
}
...
void *my_malloc(size_t s)
{
    void *rv;
    if (!(rv = malloc(s)))
        ftpc_err(ERR_HEAP_ALLOCATION_FAILED, "");
    return rv;
}
