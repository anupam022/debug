...
struct ftpc_options o = {0, 21, 8192, 1, 0, 0, 0, 0, 0, 0};
/* program options */

int ftpc_control = 0; /* control connection */
int ftpc_data = 0;    /* data connection */
...
struct termios ftpc_termios; /* terminal data */
...
{
  if (!o.q)
    printf("\r%s:%s>", mode == CMDFTP_LOCAL ? localhost : o.hostname, o.P ? "" : cwd[mode]);
  fflush(stdout);
}
...
int main(int argc, char *argv[])
{
  ...
  intro();
  reset_cmd_buffer();
  buffer = my_malloc(o.b);

  tcgetattr(0, &ftpc_termios);
  atexit(&ftpc_canon_mode);
  init_signals();
  init_temp();

  if (!(ftpc_control = ftpc_connect(o.p)))
    ftpc_err(CMDFTP_ERR_CONN, "");
...
void getoptions(int argc, char *argv[])
{
              ...
              ) != -1)
  {
    ...
    case 'a':
      rv = sscanf(optarg, "%hu", &o.a);
      break;
    case 'p':
      rv = sscanf(optarg, "%hu", &o.p);
      break;
    case 'b':
      rv = sscanf(optarg, "%hu", &o.b);
      break;
...
void greeting(void)
{
  if (recv_answer(0, 0, 0) != 220)
    ftpc_err(CMDFTP_ERR_SERV, "(greeting)");
}
...
void login_procedure(void)
{
  ...
  if (!o.m)
  {
    for (attempt = 0; attempt < o.a; attempt++)
    {
      ...
      if (manual_login())
        return;
      ftpc_war(CMDFTP_WAR_LGIN, "");
    }
  }

  ftpc_err(CMDFTP_ERR_LGIN, "");
}
...
int login(char *user, char *pass)
{
  ...
  if (send_command(cmd_buffer, 0) == 230)
    rv = 1;
  logging_in = 0;
  return rv;
}
...
int auto_login(void)
{
  ...
  char token[WIDE_LINE_LEN];
  int state, rv;
  char *rc = ".netrc";
  state = rv = 0;

  if (!env[CMDFTP_ENV_HOME])
    return 0;
  ...
  tokenize:
    if ((l = auto_login_next_token(&ptr, token)) == -1)
      continue;
    if (l == 0)
    {
      if (!state)
      {
        if (strcmp(o.hostname, token) == 0)
          state++;
        goto tokenize;
      }
      break;
    ...
    {
      if (state && !user)
        user = my_strdup(token);
      goto tokenize;
    }
    else if (l == 3)
    {
      if (state && !pass)
        pass = my_strdup(token);
      goto tokenize;
    }
  }
  if (user && !pass)
...
void read_token(char **hay, char *store)
{
  ...
  for (i = 0;; i++)
  {
    ...
    if (c == '\\')
    {
      ...
      if (isdigit(c))
      {
        sscanf(*hay, "%3o%n", (unsigned int *)&c, &n);
        *hay += n - 1;
      }
      else if (c == 'x')
      {
        (*hay)++;
        sscanf(*hay, "%2x%n", (unsigned int *)&c, &n);
        *hay += n - 1;
      }
      else
      {
...
int dispatch(char *cmd)
{
  ...
  if (*argv[0] == 0)
    return 1; /* the empty, do nothing command */

  if ((idx = str_binsearch(argv[0])) == -1)
    ftpc_war(CMDFTP_WAR_BADC, cmd);
  else if (!cmd_functions[idx](&argv[1]))
    ftpc_war(CMDFTP_WAR_FAIL, "");
...
int local_chdir(char *arg)
{
  ...
  if ((rv = chdir(arg)) < 0)
    ftpc_war(CMDFTP_WAR_LERR, strerror(errno));
...
int local_copy(char *target, char *source)
{
  ...
  if ((total_size = local_size(s)) >= 0 && (t = fopen(target, "wb")))
  {
    ...
    op = "Copying";
    csource = clean_fn(source);
    csource_len = strlen(csource);
  ...
  start_transfer:
    ...
    while (start_pos < total_size)
    {
      ...
      if (!(bytes = fread(buffer, 1, toread, s)))
        break;
      if ((written = fwrite(buffer, 1, bytes, t)) != bytes)
        break;
    ...
    {
      if (ferror(s))
        ftpc_war(CMDFTP_WAR_READ, source);
      else if (ferror(t))
        ftpc_war(CMDFTP_WAR_WRIT, target);
      else if TRANSFER_INTERRUPTED_CHECK ("local copy", 0)
        ;
      else
      {
        ftpc_err(CMDFTP_ERR_UNER, strerror(errno));
      }
    }
    else
    {
...
int local_fetch_pretty_list(struct list_data *d)
{
  ...
  if (!ftpc_execute("/bin/ls -l", 0, -1, pipe_fd[1]))
  {
    close(pipe_fd[0]);
    close(pipe_fd[1]);
...
int local_print(char *arg)
{
  if (env[CMDFTP_ENV_PAGER])
    return ftpc_execute(env[CMDFTP_ENV_PAGER], arg, -1, -1);
  else
  {
    FILE *f;
    int c;
...
{
  return ftpc_execute(env[CMDFTP_ENV_EDITOR], arg, -1, -1);
}
...
{
  char *start, *end;
  if (!send_command("PWD", 1))
    return 0;
  if ((end = strrchr(cmd_buffer, '"')) == 0 ||
      (start = strchr(cmd_buffer, '"')) == 0)
    return 0;
  *end = 0;
  start++;
  return start;
...
int remote_move(char *to, char *from)
{
  ...
  if (send_command(cmd_buffer, 0) != 350)
    return 0;

  snprintf(cmd_buffer, CMD_BUF_SIZE, "RNTO %s", to);
  if (send_command(cmd_buffer, 0) != 250)
    return 0;
...
start_transfer:
  ...
  if (!send_command("TYPE A", 0))
    goto end_proc;

  if (!(port = getport()))
    goto end_proc;
  if (!(ftpc_data = ftpc_connect(port)))
    goto end_proc;
  ...
  while ((line = recv_line(ftpc_data)))
  {
    if (!pretty)
    {
      char *fn;
...
off_t remote_size(char *filename)
{
  ...
  if (!(rv = send_command(cmd_buffer, 1)))
    return -2;
  else if (rv != 213)
    return -1;
...
int remote_print(char *arg)
{
  ...
  if (download(f, arg))
  {
    if (env[CMDFTP_ENV_PAGER])
    {
      fclose(f);
      f = 0;
      rv = ftpc_execute(env[CMDFTP_ENV_PAGER], fn, -1, -1);
    }
    else
    {
      fseeko(f, 0, SEEK_SET);
...
int remote_edit(char *arg)
{
  ...
  if (download(f, arg))
  {
    fclose(f);
    f = 0;
    if (ftpc_execute(env[CMDFTP_ENV_EDITOR], fn, -1, -1))
    {
      if ((f = fopen(fn, "r")) && upload(arg, f))
        rv = 1;
    }
  }
  if (f)
...
int do_copy_dir(char *target_dir, char *source_mask)
{
  ...
  if (!mf[mode][MODE_FETCH_LIST](source_mask, &d))
    return 0;
  if (d.count == 0)
    return 1; /* nothing to do but ok */
  ...
  for (i = 0; i < d.count; i++)
  {
             ...
             (st != FILE_ISREG && st != FILE_ISDIR))
    {
      ...
      ftpc_war(CMDFTP_WAR_SKIP, d.data[i].fullname);
    }
    ...
    else /* (st == FILE_ISDIR) */
    {
      ...
      if (!do_copy_dir(target, new_source))
        rv = 0;
      free(new_source);
    }
    free(target);
...
int cmd_cp(char **paths)
{
  ...
  if (!mf[mode][MODE_FETCH_LIST](source[2], &ld))
    goto end_proc;

  if (ld.count == 0)
  {
    ftpc_war(CMDFTP_WAR_MASK, paths[0]);
    rv = 1;
    goto end_proc;
  }

  if ((target_type = mf[mode][MODE_FILE](target[2])) < 0)
    goto end_proc;
  ...
  if (ld.count == 1)
  {
    ...
    else if (source_type == FILE_ISREG)
    {
      ...
      {
        ftpc_war(CMDFTP_WAR_TRG, "exists (not a regfile or a dir)");
      }
    }
    ...
    {
      ftpc_war(CMDFTP_WAR_SRC, "not a regular file or a dir");
    }
  }
  else if (target_type == FILE_ISDIR)
  {
  ...
  {
    ftpc_war(CMDFTP_WAR_TRG, "multiple source needs a dir target");
  }
...
int cmd_mv(char **paths)
{
  ...
  if (ld.count == 1)
  {
    ...
    else if (source_type == FILE_ISDIR)
    {
      ...
      {
        ftpc_war(CMDFTP_WAR_TRG, "exists (not a dir)");
      }
    }
    else
    {
...
int do_ren(char *mask, char *from, char *to)
{
  ...
  if (!mf[mode][MODE_FETCH_LIST](mask, &ld))
    return 0;
  if (ld.count == 0)
  {
    ftpc_war(CMDFTP_WAR_MASK, mask);
    return 1;
  }
...
int cmd_h(char **argv)
{
  ...
  if (*arg != 0)
    ftpc_war(CMDFTP_WAR_IARG, arg);

  for (i = 0; i < size_msg_help; i += 2)
...
int cmd_rm(char **argv)
{
  ...
  init_list(&d);
  if (!mf[mode][MODE_FETCH_LIST](arg, &d))
    return 0;
  if (d.count == 0)
  {
    ftpc_war(CMDFTP_WAR_MASK, arg);
    return 1;
  }
...
int cmd_dir(char **argv)
{
  ...
  init_list(&d);
  if (!mf[mode][MODE_FETCH_PRETTY_LIST](&d))
    return 0;
  if (d.count == 0)
  {
    ftpc_war(CMDFTP_WAR_MASK, "");
    return 1;
  }
...
int cmd_e(char **argv)
{
  ...
  {
    ftpc_war(CMDFTP_WAR_MENV, "EDITOR");
    return 0;
  }
...
int cmd_u(char **paths)
{
  ...
  init_list(&ld);
  if (!local_fetch_list(paths[0], &ld))
    return 0;
...
int u_aux(char *target_dir, char *source_mask)
{
  ...
  if (!local_fetch_list(source_mask, &ld))
    return 0;
  if (ld.count == 0)
    return 1;
  ...
  for (i = 0; i < ld.count; i++)
  {
    ...
    if (file_type == FILE_ISREG)
    {
      ...
      if (!f)
        ftpc_war(CMDFTP_WAR_OPEN, ld.data[i].fullname);
      else
      {
        char *fn;
    ...
    {
      ftpc_war(CMDFTP_WAR_SKIP, ld.data[i].fullname);
    }
    if (!rv)
      break;
  }
...
int upload(char *target, FILE *source)
{
  ...
  op = "Uploading";
  ctarget = clean_fn(target);
  ctarget_len = strlen(ctarget);

start_transfer:

  if (!send_command("TYPE I", 0))
    return 0;
  if ((total_size = local_size(source)) < 0)
    return 0;
  ...
  if (send_command(cmd_buffer, 0) != 150)
  {
    close(ftpc_data);
    return 0;
  }
  ...
  while (start_pos < total_size)
  {
    ...
    if (!(bytes = fread(buffer, 1, toread, source)))
      break;
    if ((written = my_raw_write(buffer, bytes, ftpc_data)) != bytes)
      break;
  ...
  {
    if (ferror(source))
      ftpc_war(CMDFTP_WAR_READ, basename(target));
...
int do_d(char **paths, int resume)
{
  ...
  init_list(&ld);
  if (!remote_fetch_list(paths[0], &ld))
    return 0;
...
int d_aux(char *target_dir, char *source_mask, int resume)
{
  ...
  if (!remote_fetch_list(source_mask, &ld))
    return 0;
  if (ld.count == 0)
    return 1;
...
start_transfer:
  ...
  if (!send_command("TYPE I", 0))
    return 0;
  if ((total_size = remote_size(source)) < 0)
    return 0;
  if ((start_pos = ftello(target)) == -1)
    return 0;
  ...
  while (start_pos < total_size)
  {
    ...
    if ((bytes = my_raw_read(buffer, toread, ftpc_data)) <= 0)
      break;
    if ((written = fwrite(buffer, 1, bytes, target)) != bytes)
      break;
  ...
  {
    if (ferror(target))
      ftpc_war(CMDFTP_WAR_WRIT, basename(source));
    else if TRANSFER_INTERRUPTED_CHECK ("data transfer", 1)
      ;
    else
    {
...
int ls(struct list_data *d)
{
  ...
  fclose(target);
  rv = ftpc_execute(env[CMDFTP_ENV_PAGER], 0, pipe_fd[0], -1);
...
int ftpc_connect(int port)
{
  ...
  if (!server)
    if (!(server = gethostbyname(o.hostname)))
      return 0;
...
start_transfer:
  ...
  if (my_raw_write(s, len, ftpc_control) < 0)
  {
    s[len - 2] = 0;
...
char *recv_line(int sc)
{
  ...
  if (*cmd_ptr == 0)
  {
    ...
    cmd_buffer[bytes] = 0;
    cmd_ptr = cmd_buffer;
    if (*cmd_ptr == '\n' && cmd_buffer[CMD_BUF_SIZE - 1] == '\r')
      cmd_ptr++;
  }

  new_ptr = strchr(cmd_ptr, '\r');
  if (!new_ptr)
    new_ptr = cmd_buffer + CMD_BUF_SIZE;

  n = new_ptr - cmd_ptr;
  memset(cmd_line, 0, n * CMD_BUF_SIZE);
  strncpy(cmd_line, cmd_ptr, n);
  cmd_line[n] = 0;
  cmd_ptr = new_ptr;
...
int recv_answer(int store, struct line_data *d, char suppress_err)
{
  ...
  if (answer[3] == '-')
  {
    while ((answer = recv_line(ftpc_control)))
    {
      if (strncmp(answer, str_code, 4) == 0)
        break;
      if (store)
        store_line(answer, d);
      else
  ...
  {
    ftpc_war(CMDFTP_WAR_RERR, answer);
  }
...
int getport()
{
  ...
  if ((answer = send_command("PASV", 0)) != 227)
    return 0;
  ...
  if (sscanf(port_str, "%u %u %u %u %u %u", b, b + 1, b + 2, b + 3, b + 4, b + 5) != 6)
    return 0;
...
int str_binsearch(char *key)
{
  ...
  {
    mid = (low + high) / 2;
    if ((chk = strcmp(key, commands[mid])) > 0)
      low = mid + 1;
    else if (chk < 0)
      high = mid - 1;
    else
...
char *readline(int enable_tab, int enable_echo)
{
  ...
  ptr = cmd_userinput;
  memset(cmd_userinput, 0, sizeof(cmd_userinput));
  ftpc_raw_mode();
  ...
  while (1)
  {
    ...
    {
      readline_tab();
      ptr = cmd_userinput + strlen(cmd_userinput);
    }
    else if (c == ftpc_termios.c_cc[VERASE] || c == 8)
...
{
  char *argv[4];
  char *mask, *completion;
  struct list_data ld;
  int c, i, j, lc_mode;
  size_t len, len_buffer;
  ...
  if (!mf[lc_mode][MODE_FETCH_LIST](mask, &ld) || ld.count == 0)
    goto endtab;
  escape_list(&ld);
  ...
  for (i = 0; i < len; i++)
  {
    ...
    {
      if (ld.data[j].escaped_fullname[i] != c)
        goto endcommon;
    }
  }
...
endtab:
  ...
  print_prompt();
  printf("%s", cmd_userinput);

  free_list(&ld);
  free_cmd(argv);
}

void ftpc_pwd_start(void)
{
  ftpc_termios.c_lflag &= ~ECHO;
  tcsetattr(0, TCSANOW, &ftpc_termios);
}

void ftpc_pwd_end(void)
{
  ftpc_termios.c_lflag |= ECHO;
  tcsetattr(0, TCSANOW, &ftpc_termios);
}

void ftpc_raw_mode(void)
{
  ftpc_termios.c_lflag &= ~ICANON;
  ftpc_termios.c_lflag &= ~ECHO;
  *ftpc_termios.c_cc[VMIN] = 1;
  *ftpc_termios.c_cc[VTIME] = 0;
  tcsetattr(0, TCSANOW, &ftpc_termios);
}

void ftpc_canon_mode(void)
{
  ftpc_termios.c_lflag |= ICANON;
  ftpc_termios.c_lflag |= ECHO;
  tcsetattr(0, TCSANOW, &ftpc_termios);
...
int ftpc_execute(char *p1, char *p2, int read_fd, int write_fd)
{
  char **argv = 0;
  char *argptr;
  pid_t pid;
  ...
  count += (p2 ? 2 : 1);
  argv = my_realloc(argv, sizeof(char *) * count);
  argv[count - 1] = 0;
  if (p2)
    argv[count - 2] = p2;
...
void init_temp(void)
{
  ...
  {
    ftpc_temp_fn = fullpath(env[CMDFTP_ENV_TMPDIR], "ftpcXXXXXX");

#ifdef P_tmpdir
  }
  else if (is_good_tmpdir(P_tmpdir))
  {
    ftpc_temp_fn = fullpath(P_tmpdir, "ftpcXXXXXX");
#endif
  }
  else if (is_good_tmpdir("/tmp"))
  {
    ftpc_temp_fn = fullpath("/tmp", "ftpcXXXXXX");
  }
  else
  {
    ftpc_err(CMDFTP_ERR_TMPD, "");
  }

  ftpc_temp_fn_len = strlen(ftpc_temp_fn);
}
...
FILE *ftpc_temp(char **fn)
{
  ...
  *fn = 0;
  ...
  for (i = ftpc_temp_fn_len - 6; i < ftpc_temp_fn_len; i++)
    ftpc_temp_fn[i] = 'X';

  if ((i = mkstemp(ftpc_temp_fn)) < 0)
    return 0;
  if (!(rv = fdopen(i, "r+")))
    return 0;
  *fn = my_strdup(ftpc_temp_fn);
  return rv;
}
...
void free_list(struct list_data *d)
{
  ...
  if (d->data)
  {
    for (i = 0; i < d->count; i++)
    {
      if (d->data[i].escaped_fullname)
        free(d->data[i].escaped_fullname);
      if (d->data[i].fullname)
        free(d->data[i].fullname);
      if (d->data[i].basename)
        free(d->data[i].basename);
      if (d->data[i].dirname)
        free(d->data[i].dirname);
    }
    free(d->data);
    d->data = 0;
  }
